	...
	<?php if ( $attribute_page ) : ?>
		...
		<?php if ( ! empty( $term->description ) ) : ?>
			<div class="term-description">
				<?php echo wp_kses_post( wpautop( wptexturize( $term->description ) ) ); ?>
			</div>
		<?php endif; ?>
...
<article class="attribute-page-content">
	<?php echo wp_kses_post( apply_filters( 'the_content', $attribute_page->post_content ) ); ?>
</article>
<?php endif; ?>

<section class="attribute-products-section">
	<h2 class="products-made-with-title">
		<?php
		printf(
			/* translators: %s: attribute term name */
			esc_html__( 'Products made with %s', 'wc-rich-attribute-suite' ),
			esc_html( $term->name )
		);
		?>
	</h2>
